...
#
#   sh seed_study.sh          # all eight seeds
#   sh seed_study.sh 2        # the first two, for a quick check
#
# Writes per_seed.csv and summary.txt.  One run is about a minute.
# The run directories are named study_<seed>_<activation>; nothing else
# in this directory may match that pattern, or the cleanup below would
# remove it.
set -e
...
for s in $SEEDS; do
  ...
  for a in $ACTS; do
    ...
    ( cd "$d" && "$R/build/serial.out" > stdout.log 2>&1 )
  done
done
python3 "$(dirname "$0")/summarise_seed_study.py" $ACTS | tee summary.txt
