# plot.gp -- morse_refine (run the stages per README first).  Run `gnuplot plot.gp` after a run; writes plot.png.
#
...
#   output_set0001.dat  8:  x(1:6)  prediction(1:1)  target(1:1)
...
#
# The inputs are 6-dimensional, so a field cannot be drawn as a
...
plot x with lines lc rgb "gray" lw 1 notitle, \
     out u 8:7 with points pt 7 ps 0.5 notitle
